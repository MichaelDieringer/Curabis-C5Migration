...
/// </summary>
codeunit 51865 "C5 2008 Helper Functions"
{
    var
    ...
    begin
        ...
        if Format = 'yyyy/MM/dd' then
            if StrPos(DateString, 'PR') <> 0 then begin
                if evaluate(yyyy, CopyStr(DateString, 1, 4)) and evaluate(MM, CopyStr(DateString, 6, 2)) then begin
                    Date := DMY2Date(1, MM, yyyy) - 1;
                    date := Closingdate(Date);
                    exit(true);
                end;
            end else
                if evaluate(yyyy, CopyStr(DateString, 1, 4)) and evaluate(MM, CopyStr(DateString, 6, 2)) and evaluate(dd, CopyStr(DateString, 9, 2)) then begin
                    Date := DMY2Date(dd, MM, yyyy);
                    exit(true);
                end;
    ...
    begin
        ...
        // C-MID Hvorfor denne fejl? I BC behøves kombinationen ikke, så hvorfor tjekker vi for det?
        //else
    ...
    var
        C5ExchRates: Record "C5 2008 ExchRate";
