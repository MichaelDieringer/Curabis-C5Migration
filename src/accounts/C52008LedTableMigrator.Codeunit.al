...
/// </summary>
codeunit 51862 "C5 2008 LedTable Migrator"
{
    TableNo = "C5 2008 LedTable";
    SingleInstance = true;
    ...
    begin
        if RecordIdToMigrate.TableNo() <> Database::"C5 2008 LedTable" then
            exit;
    ...
    begin
        ...
        if C5LedTable.Department <> '' then
            Sender.CreateDefaultDimensionAndRequirementsIfNeeded(
                ...
                C5HelperFunctions.GetDimensionValueName(Database::"C5 2008 Department", C5LedTable.Department));
        if C5LedTable.Centre <> '' then
            Sender.CreateDefaultDimensionAndRequirementsIfNeeded(
                C5HelperFunctions.GetCostCenterDimensionCodeTxt(),
                C5HelperFunctions.GetCostCenterDimensionDescTxt(),
                C5LedTable.Centre,
                C5HelperFunctions.GetDimensionValueName(Database::"C5 2008 Centre", C5LedTable.Centre));
        if C5LedTable.Purpose <> '' then
            Sender.CreateDefaultDimensionAndRequirementsIfNeeded(
                C5HelperFunctions.GetPurposeDimensionCodeTxt(),
                C5HelperFunctions.GetPurposeDimensionDescTxt(),
                C5LedTable.Purpose,
                C5HelperFunctions.GetDimensionValueName(Database::"C5 2008 Purpose", C5LedTable.Purpose));
    end;

    /// <summary>
    /// MigrateLedgerDetails.
    /// </summary>
    /// <param name="C5LedTable">Record "C5 LedTable".</param>
    /// <param name="GLAccDataMigrationFacade">Codeunit "GL Acc. Data Migration Facade".</param>
    procedure MigrateLedgerDetails(C5LedTable: Record "C5 2008 LedTable"; GLAccDataMigrationFacade: Codeunit "GL Acc. Data Migration Facade")
    var
    ...
    local procedure ConvertAccountType(C5LedTable: Record "C5 2008 LedTable"): Option
    var
    ...
    local procedure ConvertDebitCreditType(C5LedTable: Record "C5 2008 LedTable"): Option
    var
    ...
    local procedure ConvertIncomeBalance(C5LedTable: Record "C5 2008 LedTable"): Option
    var
    ...
    local procedure ConvertExchangeRateAdjutment(C5LedTable: Record "C5 2008 LedTable"): Option
    var
